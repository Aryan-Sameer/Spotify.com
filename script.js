...
async function getSongs(folder) {
    currFolder = folder
    let a = await fetch(`https://aryan-sameer.github.io/Spotify.com/songs/${folder}`)
...
async function displayAlbums() {
    let a = await fetch(`https://aryan-sameer.github.io/Spotify.com/songs/`)
    let response = await a.text()
    let div = document.createElement('div')
    div.innerHTML = response
    ...
    for (let index = 0; index < array.length; index++) {
        ...
        if (e.href.includes("/songs/")) {
            ...
            let a = await fetch(`https://aryan-sameer.github.io/Spotify.com/songs/${folder}/info.json`)
